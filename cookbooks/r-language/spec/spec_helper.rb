...
# Stub shell_out for r_package resource. Matches the array-form shell_out
# the resource now uses (r_exe, '-e', expression).
def stub_r_package_shell_out(package_name, installed = false, version = nil)
  stubs_for_provider("r_package[#{package_name}]") do |provider|
    no_version_expr = "exit(!require('#{package_name}', quietly = TRUE))"
    allow(provider).to receive(:shell_out)
      .with('/usr/bin/Rscript', '-e', no_version_expr)
      .and_return(double(exitstatus: installed ? 0 : 1))

    if version
      versioned_expr = "exit(!require('#{package_name}', quietly = TRUE) || packageVersion('#{package_name}') != '#{version}')"
      allow(provider).to receive(:shell_out)
        .with('/usr/bin/Rscript', '-e', versioned_expr)
        .and_return(double(exitstatus: installed ? 0 : 1))
    end
  end
end
